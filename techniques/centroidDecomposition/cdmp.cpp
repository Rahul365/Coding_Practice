    ...
    int oneCentroid(int u){
        ...
        function<void(int,int)> dfs = [&](int u,int p){
            ...
            for(pair<int,int> v : g[u]){
                if(v.first == p || dead[v.first]) continue;
                dfs(v.first,u);
        ...
        function<int(int ,int)> findCentroid = [&](int u,int p){
            for(pair<int,int> v : g[u]){
                if(v.first == p || dead[v.first]) continue;
                if(2 * subtree[v.first] > N) return findCentroid(v.first,u);
            }
            return u;
        };
...
struct LCA{
    ...
    int findlca(int u,int v){
        ...
        if(u == v) return u;
        for(int i = lg;i>=0;--i)
         if(par[u][i]!=-1 && par[u][i]!=par[v][i]) u = par[u][i],v = par[v][i];
    ...
    void build(){
        level[1] = 0;
        for(int i = 1;i<maxn;++i) par[i][0]=-1;
        dfs(1,1);
        for(int i = 1;i<LG;++i){
            for(int j = 1;j<=n;++j){
                if(par[j][i-1]!=-1){
